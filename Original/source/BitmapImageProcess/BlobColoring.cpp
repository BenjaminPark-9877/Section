...
// Construction
//////////////////////////////////////////////////////////////////////
CBlobColoring::CBlobColoring(unsigned char* InImg, int height, int width)
{
	nBlob=0;
	distanceBound = 0;
	...
	for (int i = 0; i < rwsize * height; i++)
	{
		m_InImg.push_back(InImg[i]);
	}

	passImg.resize(height * width);
	fill_n(passImg.begin(), (char)0, (height * width));

	m_Coloring.resize(height * width);
	fill_n(m_Coloring.begin(), 0, height * width);

	blobData.resize(height * width);
...
{
	if ((m_stackx.size() >= m_MaxStack) || (m_stacky.size() >= m_MaxStack))
	{
		return false;
	}
...
{
	if ((m_stackx.size() <= 0) || (m_stacky.size() <= 0))
	{
		return false;
	}
...
////	for(i=0; i<rwsize*height; i++) this->m_InImg[i]=m_InImg[i];
////	

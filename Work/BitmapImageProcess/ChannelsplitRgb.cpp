...
void CChannelsplitRgb::Process(const CUserBitmap* pCbmpElementsIn)
{
	...
	for (int k = 2; k >= 0; k--)
	{
		...
		for (i = 0; i < m_pCbmpElements->GetMaximumHeight(); i++)
		{
			...
			{
				m_pCbmpElements->operator[](index + 3 * j + k) = pCbmpElementsIn->operator[](index + 3 * j + k);
			}
		}

		CopyClipboard(m_pCbmpElements->GetImage().data(), m_pCbmpElements->GetMaximumHeight(), m_pCbmpElements->GetMaximumWidth(), 24);
